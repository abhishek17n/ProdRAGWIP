...
with st.sidebar:
    ...
    # Display checkboxes in the sidebar
    if filenames:
        for filename in filenames:
            if st.sidebar.checkbox(filename, key=filename):
                selected_filenames.append(filename)
    else:
        st.sidebar.warning("No filenames found in the index.")
    
    # Display selected filenames in the main area
...
if st.button("Search"):
    if query:
        ...
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        resultsall = search_documents(query)
        results = filter_by_filenames(selected_filenames,  
