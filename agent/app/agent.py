...
# Custom CrewAI tool for search and scrape
class SearchAndScrapeTool(BaseTool):
    name: str = "SearchAndScrape"
    description: str = "Search for a relevant website using SerpAPI and scrape its content."
...
# Custom LLM wrapper for CrewAI to bypass litellm
class CustomGeminiLLM:
    def __init__(self, model="gemini-1.5-flash", temperature=0.5, api_key=None):
...
# Initialize the custom Gemini LLM for CrewAI
gemini = CustomGeminiLLM(model="gemini-pro", temperature=0.5, api_key=google_api_key)
gemini_llm = LLM(model="gemini/gemini-2.0-flash", temperature=0.5, api_key=google_api_key,verbose=True)
...
# Streamlit UI
st.title("LangChain Agent with Elasticsearch, SerpAPI, and Search Google & Gemini APi Crew AI")
...
query = st.text_input("Enter your query for Elasticsearch & Gemini Model")
serpapi_query = st.text_input("Google search results(Serpapi ) & Gemini API ")
crew_task = st.text_input("Perplixity Mode (Serpapi & Reasearch and Writer with Google Gemini  ) Crew AI")
